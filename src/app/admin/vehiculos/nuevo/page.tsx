...
export default function NuevoVehiculoPage() {
  const [fotos, setFotos] = useState<{ url: string; storagePath: string }[]>([])
  ...
  return (
    <div>
      ...
      <form action={async (formData) => {
        setEnviando(true)
        try {
          await crearVehiculoConFotos(formData, fotos)
        } finally {
          setEnviando(false)
        }
        ...
        <div className="flex gap-4 pt-4">
          <button type="submit" disabled={enviando} className="bg-blue-600 text-white px-6 py-2 rounded hover:bg-blue-700 disabled:opacity-50">
